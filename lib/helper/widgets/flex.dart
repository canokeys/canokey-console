...
import 'package:canokey_console/helper/widgets/flex_item.dart';
import 'package:canokey_console/helper/widgets/list_extension.dart';
...
class MyFlex extends StatelessWidget {
  ...
  getPadding(index, length) {
    if (contentPadding) {
      ...
      return Spacing.fromLTRB(index == 0 ? 0 : (spacing ?? flexSpacing) / 2, 0, index == length - 1 ? 0 : (spacing ?? flexSpacing) / 2, 0);
    }
  }
      ...
      builder: (BuildContext context, BoxConstraints constraints, screenMediaType) {
        ...
        for (List<MyFlexItem> lCol in getGrouped(screenMediaType)) {
          list.addAll(lCol.mapIndexed((index, col) => Container(
                width: getWidthFromFlex(width, col.flex[screenMediaType] ?? ScreenMedia.flexColumns, lCol.length, spacing ?? flexSpacing),
  ...
  List<List<MyFlexItem>> getGrouped(ScreenMediaType type) {
  ...
  double getWidthFromFlex(double width, int flex, int items, double spacing) {
    return (width * flex / ScreenMedia.flexColumns).floorToDouble();
  }
}
